	...
	canvas.on({
		...
		'mouse:move': function (o) {
			if (isMouseDown && isDrawing) {
				...
				for (var i = 0; i < currentObj.points.length; i++) {
					if (currentObj.points[i].name === "P-2"){
						currentObj.points[i].left = pointer.x;
						currentObj.points[i].top = pointer.y;
					}
				}
				currentObj.Draw();
		...
		'mouse:up': function (o) {
			if (isDrawing) {
				currentObj.Remove();
				currentObj.Draw();
				Shapes.push(currentObj);
				canvas.renderAll();
			}
			if(currentObj){
				currentObj.isMoving = false;
			}
			loadDataToTable();
			panning = false;
			isDrawing = false;
...
function loadDataToTable() {
	...
	for (var i = 0; i < Shapes.length; i++) {
		...
		{
			context += '<tr><td><i class="showDetail fa fa-plus" style="font-size:10pt" onclick="ShowShapeDetail(this,event);"></i></td><td class="name"><input id="name" onclick="onInputClicked(this);" onkeypress="return ValidateKey();" type="text" size="5" style="border:none" onchange="textChange(this,this.value);" value="' + Shapes[i].name
			+'" /></td><td>'+Shapes[i].type+'</td><td></td><td><span class="table-remove fa fa-trash-o"></span></td></tr>' +
			...
			Shapes[i].points.forEach(function(p){
				...
				context += '<tr onclick="showPointDetail(this);"><td hidden="true">'+Shapes[i].name+'</td><td>'+p.name+'</td><td>'+ x +'</td><td>'+ y +'</td></tr>';
			});
