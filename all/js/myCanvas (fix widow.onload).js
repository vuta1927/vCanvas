...
var vcanvas = /** @class */ (function () {
                ...
                if (img.height > 0 && img.width > 0) {
                    ...
                    for (var i = 0; i < RawData.shapes.length; i++) {
                        ...
                        if (s.type != types.Text) {
                            ...
                            var middlePoint1, middlePoint2;
                            s.middlePoints.forEach(p => {
                                if (p.name == "MP-1"){
                                    middlePoint1 = p;
                                }else if(p.name == "MP-2"){
                                    middlePoint2 = p;
                            ...
                            if (s.type == types.TwoAreaRectH) { // p1, p2, middlePoint1, p3, p4, middlePoint2
                                newShape.points[2].index = 3;
                                newShape.points[3].index = 4;
                        
                                var newPoint1 = new Point({
                                    name: middlePoint1.name,
                                    parentName: newShape.name,
                                    left: GetValueFromPercent(middlePoint1.left, img.width),
                                    top: GetValueFromPercent(middlePoint1.top, img.height),
                                    index: 2,
                                    middlePoint: true
                                });

                                if (s.type === types.HLine) {
                                    newPoint1.lockMovementX = false;
                                    newPoint1.lockMovementY = true;
                                } else if (s.type === types.VLine) {
                                    newPoint1.lockMovementX = true;
                                    newPoint1.lockMovementY = false;
                                } else if (s.type === types.Line || s.type === types.Rect) {
                                    newPoint1.lockMovementX = false;
                                    newPoint1.lockMovementY = false;
                                }
                                ...
                                var newPoint2 = new Point({
                                    name: middlePoint2.name,
                                    parentName: newShape.name,
                                    left: GetValueFromPercent(middlePoint2.left, img.width),
                                    top: GetValueFromPercent(middlePoint2.top, img.height),
                                    index: 5,
                                    middlePoint: true
                                });

                                if (s.type === types.HLine) {
                                    newPoint2.lockMovementX = false;
                                    newPoint2.lockMovementY = true;
                                } else if (s.type === types.VLine) {
                                    newPoint2.lockMovementX = true;
                                    newPoint2.lockMovementY = false;
                                } else if (s.type === types.Line || s.type === types.Rect) {
                                    newPoint2.lockMovementX = false;
                                    newPoint2.lockMovementY = false;
                                }
                                ...
                                newShape.points.splice(2, 0, newPoint1);
                                newShape.points.push(newPoint2);
                                newShape.middlePoints.push(newPoint1);
                                newShape.middlePoints.push(newPoint2);
                            } else if (s.type == types.TwoAreaRectV) { //p1, middlePoint1, p2, p3, middlePoint2, p4
                                newShape.points[1].index = 2;
                                newShape.points[3].index = 5;
                                var newPoint1 = new Point({
                                    name: middlePoint1.name,
                                    parentName: newShape.name,
                                    left: GetValueFromPercent(middlePoint1.left, img.width),
                                    top: GetValueFromPercent(middlePoint1.top, img.height),
                                    index: middlePoint1.index,
                                    middlePoint: true
                                });
                                ...
                                newShape.points.splice(1, 0, newPoint1);
                                newShape.points.splice(4, 0, newPoint2);
                                newShape.middlePoints.push(newPoint1);
                                newShape.middlePoints.push(newPoint2);
                            }
    ...
    vcanvas.prototype.initEvent = function () {
        ...
        console.log("canvas width: " + c.width, "div width: " + $("#" + mother.id + '-wrapper').width())
        if (c.width != $("#" + mother.id + '-wrapper').width()) {
            var scaleMultiplier = $("#" + mother.id + '-wrapper').width() / c.width;
            var objects = c.getObjects();
            originWidth = c.getWidth()
            originHeight = c.getHeight()
        ...
        context = `${mother.shapes.map(s =>
            ...
            <td class="text-center"  data-toggle="collapse" data-target="#${mother.id + '-'+s.name+'tr-pointDetail'}">
                ...
                ${this.extColumns.map(c => `
                    ...
                    <td><input id="${mother.id + '-' + s.name + '-checkbox-' + c.name}"  type="checkbox" style="width:${c.width?c.width+'px':'50px'};padding:0px;margin:0px" ${s[c.name]!=null? `checked`:``} />
                    </td>`: ``}
                `)}
                `}
            ...
            shape.points.forEach(function (point) {
                if (!point.middlePoint) {
                    ShapeExportObj.points.push({
                        ...
                        isMiddlePoint: point.middlePoint
                    });
                    count++;
                }
            });

            shape.middlePoints.forEach(point => {
...
var Shape = /** @class */ (function () {
    ...
    Shape.prototype.TwoAreaRectHorizontal = function () {
        ...
        this.points = [p1, p2, middlePoint1, p3, p4, middlePoint2];
    }
    ...
    Shape.prototype.TwoAreaRectVertical = function () {
        ...
        this.middlePoints = [middlePoint1, middlePoint2];
        this.points = [p1, middlePoint1, p2, p3, middlePoint2, p4];
    }
    ...
    Shape.prototype.Draw = function (params) {
        ...
        if (this.type != types.Text) {
            ...
            if (this.type == types.Line) {
                ...
                this.lines = arrLine;
            } else if (this.type == types.TwoAreaRectH || this.type == types.TwoAreaRectV) {
                for (var i = 0; i < this.points.length; i++) {
                    var line;
                    if (i !== (this.points.length - 1)) {
                        line = new fabric.Line([
                ...
                line = new fabric.Line([this.middlePoints[0].left, this.middlePoints[0].top, this.middlePoints[1].left, this.middlePoints[1].top], {
                    name: "Mline",
                    parentName: this.name,
                    fill: this.color,
                    stroke: this.color,
        ...
        if (this.type != types.Text) {
            ...
            if (point) {
                if (point.middlePoint) {
                    var middlePoint1;
                    var  middlePoint2;
                    this.middlePoints.forEach(p => {
                        if (p.name == "MP-1") middlePoint1 = p;
                        else if (p.name == "MP-2") middlePoint2 = p;
                    });
                    if (this.type == types.TwoAreaRectH) { // p1, p2, middlePoint1, p3, p4, middlePoint2
                        if (this.points[0].top < point.top && point.top < this.points[4].top) {
                            if (point.name == 'MP-2') {
                                if (point.top < this.points[3].top && point.top > this.points[1].top && point.left < middlePoint1.left) {
                                    this.points[5].left = point.left;
                                    this.points[5].top = point.top;

                                    this.points[2].top = point.top;
                                }

                            } else if (point.name == 'MP-1') {
                                if (point.top < this.points[3].top && point.top > this.points[1].top && point.left > middlePoint2.left) {
                                    this.points[2].left = point.left;
                                    this.points[2].top = point.top;
                    ...
                    } else if (this.type == types.TwoAreaRectV) { // p1, middlePoint1, p2, p3, middlePoint2, p4
                        if (this.points[0].left < point.left && point.left < this.points[2].left) {
                            if (point.left > this.points[5].left && point.left < this.points[3].left) {
                                if (point.name == 'MP-1' && point.top < middlePoint2.top) {
                                    this.points[1].left = point.left;
                                    this.points[1].top = point.top;

                                    this.points[4].left = point.left;
                                } else if (point.name == 'MP-2' && point.top > middlePoint1.top) {
                                    this.points[4].left = point.left;
                                    this.points[4].top = point.top;
    ...
    Shape.prototype.GetMinMaxCoodr = function () {
        var ymin = ymax = this.points[0].top;
        var xmin = xmax = this.points[0].left;

        for (var i = 0; i < this.points.length; i++) {
            if (ymin > this.points[i].top) {
                ymin = this.points[i].top;
            }
            if (ymax < this.points[i].top) {
                ymax = this.points[i].top;
            }

            if (xmin > this.points[i].left) {
                xmin = this.points[i].left;
            }

            if (xmax < this.points[i].left) {
                xmax = this.points[i].left;
            }
        }
